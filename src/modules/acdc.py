...
class SaveActivations(ContextDecorator):
    def __init__(self, modules: List[Tuple[nn.Module, str]], verbose=False):
        '''
        Args:
    ...
    def get_activations(self):
        return {name:activation for (module, name), activation in zip(self.modules, self.activations)}

    def save_outputs(self, module, input, output):
        self.activations.append(output)
        if self.verbose:
            print(f"Saving output from {module.__class__.__name__}")
    ...
    def __enter__(self):
        ...
        for module, name in self.modules:
            ...
            if self.verbose: print(f"hooking {module.__class__.__name__}: {name}")
        return self
